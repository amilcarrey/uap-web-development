...
export default function TaskList({ tareas, onToggle, onDelete, loading, error, userRole }: Props) {
  ...
  return (
    <div className="mt-6">
      {tareas.map((t, index) => (
        <div
          key={t.id}
          className={`flex justify-between items-center py-2 ${index !== 0 ? "border-t border-gray-200" : ""}`}
        >
          <button
