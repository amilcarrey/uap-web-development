...
export default function ReviewList({ bookId }: { bookId: string }) {
  ...
  useEffect(() => {
    setReviews(getReviews(bookId).sort((a, b) => b.votes - a.votes)); // Ordena por votos
  }, [bookId]);
      ...
      {reviews.map((review) => (
        <div key={review.id} className="border p-4 mb-4">
          ...
          <div className="flex mt-2">
            <button onClick={() => handleVote(review.id, 1)} className="mr-2">
              <FaThumbsUp /> {review.votes > 0 ? `+${review.votes}` : review.votes}
            </button>
            <button onClick={() => handleVote(review.id, -1)}>
              <FaThumbsDown />
            </button>
          </div>
        </div>
