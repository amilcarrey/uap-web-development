...
export default function Configuraciones() {
  ...
  const handleIntervaloSubmit = (e: React.FormEvent) => {
    ...
    actualizarMutation.mutate({ intervaloRefetch: nuevoIntervalo }, {
      onSuccess: () => mostrarToast('Intervalo de refetch actualizado', 'exito'),
  ...
  return (
    <div className="flex flex-col items-center space-y-6 max-w-md mx-auto">
      <div className="bg-pink-400 p-6 rounded-lg shadow-md w-full">
        <h3 className="text-lg font-semibold mb-4 text-center">Intervalo de Refetch</h3>
        <form onSubmit={handleIntervaloSubmit} className="flex flex-col space-y-3">
          <div className="flex items-center space-x-2">
            ...
            />
            <span className="text-black-600">segundos</span>
          </div>
          <button
            type="submit"
            disabled={actualizarMutation.isPending}
          ...
          >
            {actualizarMutation.isPending ? 'Actualizando...' : 'Actualizar Intervalo'}
          </button>
        </form>
      </div>
