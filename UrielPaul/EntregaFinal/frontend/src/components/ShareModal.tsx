...
import { api } from "../api"
import { XMarkIcon, UserPlusIcon, UserIcon, StarIcon, PencilIcon, EyeIcon } from "@heroicons/react/24/outline"
...
export default function ShareModal({ boardId, onClose }: Props) {
  ...
  const changeRole = (userId: number, newRole: ShareItem["role"]) =>
    api(`/boards/${boardId}/share/${userId}`, {
      method: "PATCH",
      body: JSON.stringify({ role: newRole }),
    }).then(load)

  const removeUser = (userId: number) => api(`/boards/${boardId}/share/${userId}`, { method: "DELETE" }).then(load)
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${styles[role as keyof typeof styles]}`}
      >
        {getRoleIcon(role)}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-800 w-full max-w-md transition-colors duration-300">
        {/* Header */}
        ...
        <div className="p-6 border-b border-gray-200 dark:border-gray-800">
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-red-500 dark:focus:ring-red-400 focus:border-transparent transition-all duration-200 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                value={role}
        ...
        <div className="p-6">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">
            Miembros del tablero ({list.length})
          </h3>
          ...
          {list.length > 0 ? (
            <div className="space-y-3 max-h-60 overflow-y-auto">
              {list.map((user) => (
                <div
                  key={user.userId}
                  className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors duration-300"
                >
                  <div className="flex items-center space-x-3">
