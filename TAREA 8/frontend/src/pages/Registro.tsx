...
const Registro: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await registro(nombre.trim(), email, contraseña);
      showSuccess('¡Cuenta creada!', 'Tu cuenta se ha creado correctamente');
      navigate('/dashboard');
    } catch (err: any) {
      const errorMsg = err.response?.data?.error || 'Error al registrar usuario';
      setErrors({ general: errorMsg });
      showError('Error de registro', errorMsg);
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (errors.confirmarContraseña) setErrors(prev => ({ ...prev, confirmarContraseña: undefined }));
                  }}
                  className={`mt-1 appearance-none relative block w-full px-3 py-2 pr-10 border placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 focus:z-10 sm:text-sm ${
